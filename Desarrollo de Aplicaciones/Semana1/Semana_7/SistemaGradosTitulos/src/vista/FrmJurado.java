...
public class FrmJurado extends JFrame {
    ...
    public FrmJurado() {
        ...
        cargarTabla();
        cargarCombos();
        setTitle("Jurados de Sustentación - UPLA");
        setSize(750, 550);
    ...
    private void initComponents() {
        ...
        add(header);
        JLabel lbl = new JLabel("JURADOS DE SUSTENTACIÓN", SwingConstants.CENTER);
    ...
    private void cargarCombos() {
        // Cargar proyectos disponibles (con estado Aprobado_Por_Asesor o Aprobado_Por_Jurado)
    ...
    private void cargarTabla() {
        ...
        try {
            String sql = "SELECT j.id_asignacion, p.id_proyecto, p.titulo, u.nombres, u.apellidos, j.rol, j.resolucion_asignacion " +
                         ...
                         "JOIN proyecto p ON j.id_proyecto = p.id_proyecto " +
                         "JOIN usuario u ON j.codigo_docente = u.codigo " +
                         "ORDER BY j.id_asignacion DESC";
            ResultSet rs = con.createStatement().executeQuery(sql);
            while (rs.next()) {
                String titulo = rs.getString("titulo");
                if (titulo.length() > 40) titulo = titulo.substring(0, 40) + "...";
