...
public class ProyectoController {
    ...
    // CORREGIDO - Muestra TODOS los proyectos (no solo los aprobados)
    ...
    // NUEVO - Lista proyectos para asesores (todos los que no tienen asesor o están activos)
    public List<String> listarProyectosParaAsesor() {
        List<String> lista = new ArrayList<>();
        try {
            String sql = "SELECT p.id_proyecto, p.titulo, u.nombres, u.apellidos " +
                         "FROM proyecto p " +
                         "JOIN tramite t ON p.id_tramite = t.id_tramite " +
                         "JOIN usuario u ON t.codigo_estudiante = u.codigo " +
                         "WHERE p.estado NOT IN ('Sustentado') " +
                         "ORDER BY p.id_proyecto DESC";
            ResultSet rs = con.createStatement().executeQuery(sql);
            while (rs.next()) {
                String titulo = rs.getString("titulo");
            ...
            System.out.println("Error listarProyectosParaAsesor: " + e.getMessage());
        }
        return lista;
    }
