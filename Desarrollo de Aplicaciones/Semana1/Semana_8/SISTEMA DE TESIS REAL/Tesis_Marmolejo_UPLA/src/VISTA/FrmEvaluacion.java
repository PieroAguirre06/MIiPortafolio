...
public class FrmEvaluacion extends JFrame {
    ...
    public FrmEvaluacion() {
        ...
        cargarTabla();
        cargarCombos();
        setTitle("Evaluaciones de Proyectos - UPLA IBERCAP");
        setSize(900, 620);
    ...
    private void initComponents() {
        ...
        add(header);
        JLabel lbl = new JLabel("GESTIÓN DE EVALUACIONES DE PROYECTOS", SwingConstants.CENTER);
        ...
        agregarLabel("Evaluador:", 620, y);
        cmbEvaluador = new JComboBox<>();
        cmbEvaluador.setBounds(700, y, 170, 25);
        add(cmbEvaluador);

        y = 90;
        agregarLabel("Rúbrica:", 20, y);
        cmbRubrica = new JComboBox<>();
        cmbRubrica.setBounds(100, y, 320, 25);
        add(cmbRubrica);

        agregarLabel("Etapa:", 450, y);
        cmbEtapa = new JComboBox<>(new String[]{"Revision_Plan", "Revision_Borrador_Final", "Sustentacion"});
        cmbEtapa.setBounds(510, y, 150, 25);
        add(cmbEtapa);

        agregarLabel("Condición Final:", 690, y);
        ...
        cmbCondicion.setBounds(700, y, 170, 25);
        add(cmbCondicion);
        ...
        scrollCom.setBounds(450, y, 420, 60);
        add(scrollCom);
        ...
        scroll.setBounds(10, 250, 875, 320);
        add(scroll);
        ...
        try {
            ...
            if (rs.next()) {
                ...
                for (int i = 0; i < cmbEvaluador.getItemCount(); i++) {
                    if (cmbEvaluador.getItemAt(i).startsWith(codigoEvaluador + " -")) {
                        cmbEvaluador.setSelectedIndex(i);
                        break;
                    }
                }
                for (int i = 0; i < cmbRubrica.getItemCount(); i++) {
                    if (cmbRubrica.getItemAt(i).startsWith(idRubrica + " -")) {
    ...
    private void cargarTabla() {
        ...
        try {
            String sql = "SELECT e.id_evaluacion, p.id_proyecto, p.titulo, u.nombres, u.apellidos, r.nombre, e.etapa, e.puntaje_obtenido, e.condicion_final, e.fecha_evaluacion " +
                         "FROM evaluacion_cabecera e JOIN proyecto p ON e.id_proyecto = p.id_proyecto " +
                         "JOIN usuario u ON e.codigo_evaluador = u.codigo " +
                         "JOIN rubrica_catalogo r ON e.id_rubrica = r.id_rubrica ORDER BY e.id_evaluacion DESC";
